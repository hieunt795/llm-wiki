...
def main():
    ...
    graph_data["nodes"] = dedupe_nodes(graph_data["nodes"])
    graph_data["links"] = dedupe_links(graph_data["links"])

    with open(GRAPH_JSON, "w", encoding="utf-8") as f:
        json.dump(graph_data, f, indent=2)

    print(
        "Successfully injected {} nodes/rationales and {} edges into the graph."
        .format(len(new_nodes), len(new_edges))
    )
